...
module.exports = function(router){
    router.get('/customers', employee,async (req, res, next) => {
        return await tryCatch(async () => {
            ...
            const list = await customersSchema.find(q).lean();
            await mongo.close();
        ...
        return await tryCatch(async () => {
            ...
            await upload(req, res, async function(err) {
                ...
                if(result && result.data.errorCode == 0){
                    ...
                    const checkIn = new checkInSchema({cusId, roomId})
                    await checkIn.save()


                    const qr_code = await utils.generateQRCode({ cusName: isBooking.cusId.cusName, phoneNumber: isBooking.cusId.phoneNumber, roomNo: isBooking.roomId.roomNo, roomId: isBooking.roomId._id  })
